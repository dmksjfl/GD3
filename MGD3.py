...
class MGD3(object):
	...
	def train(self, timestep, replay_buffer, batch_size=100):
		self.train_one_q_and_pi(timestep, replay_buffer, update_q1=True, batch_size=batch_size)
		self.train_one_q_and_pi(timestep, replay_buffer, update_q1=False, batch_size=batch_size)
	...
	def train_one_q_and_pi(self, timestep, replay_buffer, update_q1, batch_size=100):
		...
		with torch.no_grad():
			...
			if self.weighting:
				gd3_next_Q_1 = self.generalized_activation_opertator(beta = self.first_beta, 
                                                                importance_sampling = self.with_importance_sampling,
                                                                operator = self.first_operator)
			
				gd3_next_Q_2 = self.generalized_activation_opertator(beta = self.second_beta, 
                                                                importance_sampling = self.with_importance_sampling,
                                                                operator = self.second_operator)
				prop = 0.001 * (1 + timestep//1000)  ## every 1000 steps update weighting
				activate_next_Q = prop * gd3_next_Q_1(next_Q, noise_pdf) + (1 - prop) * gd3_next_Q_2(next_Q, noise_pdf)
			else:
				if timestep <= self.end_mix:
					gd3_next_Q = self.generalized_activation_opertator(beta = self.first_beta,
                                                                       importance_sampling = self.with_importance_sampling,
                                                                       operator = self.first_operator)
				else:
					gd3_next_Q = self.generalized_activation_opertator(beta = self.second_beta,
                                                                       importance_sampling = self.with_importance_sampling,
                                                                       operator = self.second_operator)
